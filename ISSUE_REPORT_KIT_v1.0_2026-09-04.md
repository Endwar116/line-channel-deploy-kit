...
| J | 附件功能預設全鎖，連部署者本人都無法傳檔；作者已定調為缺陷卻未套用到預設值 | 預設值缺陷 | **是**，附件功能不可用 |
| K | `s_tier_channels` 在兩支程式中語意相反，修好附件即打壞回報 | 設定語意衝突 | **是**，兩功能無法並存 |
...
#### J. 附件功能預設完全封閉，連部署者本人都無法傳檔；且作者已定調為設計缺陷卻未於安裝器修正

**嚴重度：高　阻斷性：是（附件功能完全不可用）**

安裝器建立的兩項設定，使附件功能在預設狀態下對**所有人**關閉，包含部署者本人：
...
`handle_attachment()` 的閘門有兩道，非 S 級通道兩道都要過：

1. 寄件者 uid 須在 `attach_whitelist.txt`（預設空）
2. 檔名須先於「任務」訊息中點名（點名制）

實測部署者於私訊傳送兩份 .docx：

```
17:43:35 ATTACH_REJECTED 非白名單 Ua4adbf6 '黃昱捷_AI職能培訓系列_V2_系統整合實戰版_完整課綱.docx'
17:43:39 ATTACH_REJECTED 非白名單 Ua4adbf6 '黃昱捷_AI職能培訓系列_產投合作完整課綱.docx'
```

**本項最值得注意之處**：`core/line_bridge.py` 的原始碼註解顯示，
作者已親身遭遇並將其定調為設計缺陷：

```python
# ── v1.12 S 級豁免（0821 S級成員 傳檔被白名單拒收→08-24 R695 定為設計缺陷；
#    SESSION整理 P 列 #6）──
```

亦即：問題已被發現、已被定性、已實作豁免機制（S 級通道）——
**但安裝器未將部署者的私訊通道預設列為 S 級**，
於是每一位新部署者都會再踩一次同一個坑。

修復已實作但未套用到預設值，是本報告中第二次出現同型問題
（第一次是缺陷 E：`skip_if_exists=True` 使既有部署無法藉重跑安裝器修復）。

**影響**

1. 附件功能預設完全不可用，且 `AGENT_SETUP.md` 全篇未提及附件需要設定。
2. 失敗訊息僅有「非白名單」四字，未指出要修改哪個檔案，部署者無從自行排除。
3. 對「把檔案傳給值台處理」這個核心使用情境而言，等同功能缺席。

**建議**

安裝器將部署者的私訊通道預設寫入 `s_tier_channels`：

```python
cfg = {..., "s_tier_channels": [],  # ← 認主完成後應自動填入主人的 uid
       ...}
```

由於安裝當下尚未認主（`line_owner.txt` 還沒有值），
建議改於 bridge 的認主流程（`OWNER_REGISTERED`）完成時自動寫入，
或於 `AGENT_SETUP.md` 增列一步說明。

同時建議將拒收訊息改為可行動的指引，例如
「附件被拒：寄件者不在白名單。請將你的 userId 加入 config/attach_whitelist.txt，
或把此通道加入 kit_config.json 的 s_tier_channels。」

---

#### K. `s_tier_channels` 在兩支程式中語意相反，設定其一必然破壞其二

**嚴重度：高（相互矛盾的設定語意）**

同一個設定鍵在兩處被賦予**方向相反**的意義：

| 檔案 | 語意 | 效果 |
|---|---|---|
| `core/line_bridge.py` | 高信任通道 | 附件**免**白名單／點名（放寬） |
| `core/relay_say.py` | 高信任職場通道 | 本體**禁止**發言（收緊） |

```python
# line_bridge.py：S 級＝信任度高，放行附件
if cid in S_TIER_ATTACH_OK and mtype in ("file", "audio"):
    ...  # 免白名單、免點名

# relay_say.py：S 級＝職場敏感，硬擋本體發言
...
**實測後果**：為解決缺陷 J（附件被拒），將部署者的私訊通道加入 `s_tier_channels`。
附件隨即正常收下：

```
17:48:06 ATTACH_SAVED S級豁免 ... '…V2_系統整合實戰版_完整課綱.docx' 56025B
17:48:07 ATTACH_SAVED S級豁免 ... '…產投合作完整課綱.docx' 52471B
```

但**本體對部署者的回報路徑同時被鎖死**：

```
$ python3 tools/relay_say.py "測試"
✗ 拒絕：目標是高信任職場通道。
```
...
`relay_say.py` 那道防護的用意是正確的——避免內部回報被原樣送進
「主人的職場群」這類有外人在場的通道。但它的判斷條件漏了一個明顯例外：
**部署者自己的私訊通道，正是內部回報應該去的地方，那裡只有他一個人。**

**建議**

1. **拆成兩個設定鍵**（根治）。例如
   `attach_trusted_channels`（放寬附件）與 `relay_forbidden_channels`（禁止回報）。
   兩者語意相反，共用一個鍵必然衝突。

2. **短期修補**：`relay_say.py` 的防護加入「主人私訊」例外。
   本案已於本地實作並驗證——主人私訊放行、群組 S 級仍正確擋下：
